'use client';
import "../Home/home.css";

export const Home = () => {
  const bkgImg = "../../../assets/bkg-home.png";
  const bkgImgShade = "../../../assets/bkg-home-shade.png";


  return (
    <>
      <div className="home-container " id="Inicio">
        <div className="left-section dark:bg-[#282828] dark:text-white">
          <h1 className="left-section-title font-bold">ALMA EN</h1>
          <p className="vertical-text">
            Alma en movimiento <br /> 10.31.2022
          </p>
          <div className="social-icons font-bold">
            <p>SIGUEME</p>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="1em"
              height="1em"
              viewBox="0 0 512 512"
            >
              <path
                fill="currentColor"
                fillRule="evenodd"
                d="M469.333 277.333H42.666v-42.666h426.667z"
                clipRule="evenodd"
              />
            </svg>
            <a href="https://www.instagram.com/antonellatiseradanza/" target="_blank" rel="noopener noreferrer">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="1.5em"
                height="1.5em"
                viewBox="0 0 24 24"
                className="social-icon-instagram"
              >
                <path
                  fill="currentColor"
                  d="M7.8 2h8.4C19.4 2 22 4.6 22 7.8v8.4a5.8 5.8 0 0 1-5.8 5.8H7.8C4.6 22 2 19.4 2 16.2V7.8A5.8 5.8 0 0 1 7.8 2m-.2 2A3.6 3.6 0 0 0 4 7.6v8.8C4 18.39 5.61 20 7.6 20h8.8a3.6 3.6 0 0 0 3.6-3.6V7.6C20 5.61 18.39 4 16.4 4zm9.65 1.5a1.25 1.25 0 0 1 1.25 1.25A1.25 1.25 0 0 1 17.25 8A1.25 1.25 0 0 1 16 6.75a1.25 1.25 0 0 1 1.25-1.25M12 7a5 5 0 0 1 5 5a5 5 0 0 1-5 5a5 5 0 0 1-5-5a5 5 0 0 1 5-5m0 2a3 3 0 0 0-3 3a3 3 0 0 0 3 3a3 3 0 0 0 3-3a3 3 0 0 0-3-3"
                />
              </svg>{" "}</a>
            <a href="https://www.behance.net/antonellatisera?fbclid=PAZXh0bgNhZW0CMTEAAaYbuSVsSiMdW8bUWpydpkDnciLSsFhNBCoY0t22Qadpek0y8CGH6-9nn5k_aem_euc-63HZLodgQPfd6CJNuw" target="_blank" rel="noopener noreferrer">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="1.5em"
                height="1.5em"
                viewBox="0 0 24 24"
                className="social-icon-behance"
              >
                <path
                  d="M19.58 12.27c-.04-.62-.25-1.09-.62-1.41c-.37-.32-.83-.48-1.38-.48c-.58 0-1.08.17-1.39.51c-.33.34-.54.8-.62 1.38m6.35-.23c.08.41.08 1 .08 1.77h-6.5c.05.9.35 1.52.94 1.88c.35.23.78.34 1.29.34c.53 0 .96-.14 1.27-.41c.2-.15.36-.35.5-.62h2.38c-.06.54-.35 1.07-.88 1.62c-.78.88-1.9 1.3-3.34 1.3c-1.19 0-2.23-.37-3.16-1.1c-.88-.73-1.34-1.92-1.34-3.57c0-1.55.41-2.75 1.23-3.55c.82-.83 1.88-1.24 3.19-1.24c.77 0 1.47.14 2.09.42c.62.28 1.14.71 1.54 1.32c.37.53.6 1.14.71 1.84M9.58 14.07c0-.65-.27-1.1-.79-1.34c-.29-.13-.71-.2-1.25-.23H4.87v3.34H7.5c.54 0 .96-.07 1.26-.22c.55-.27.82-.79.82-1.55m-4.71-3.61H7.5c.54 0 1-.1 1.32-.31c.34-.2.5-.57.5-1.09c0-.56-.22-.96-.66-1.15c-.39-.13-.88-.19-1.47-.19H4.87m6.85 4.7c.32.5.48 1.11.48 1.82c0 .76-.2 1.4-.55 1.99a3.6 3.6 0 0 1-.88.98c-.4.29-.87.51-1.41.62c-.54.11-1.12.17-1.75.17H2V5.55h6c1.53.03 2.6.45 3.23 1.33c.38.53.57 1.16.57 1.9c0 .76-.19 1.37-.57 1.83c-.23.26-.53.5-.95.71c.63.23 1.11.6 1.44 1.1m8.34-5.1h-5.01V6.07h5.01v1.25z"
                  fill="currentColor"
                />
              </svg></a>

          </div>
        </div>
        <div className="right-section ">
          <h1 className="right-section-title font-bold dark:text-white">MOVIMIENTO</h1>
          <p className="subtitle dark:text-white">Bailarina. Docente. Coreógrafa</p>
          <img
            src={bkgImg}
            alt="Bailarina en movimiento"
            className="dancer-img"
          />
          <img
            src={bkgImgShade}
            alt="Logo sombra"
            className="dancer-shade-img"
          />
        </div>
      </div>
    </>
  );
};
